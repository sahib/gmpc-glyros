...
static gpointer glyros_fetch_thread(void * data)
{
    ...
    GlyrDatabase * db = glyr_db_init((char*)"/tmp/gmpc_cache");
    ...
    */
    ...
    temp = cfg_get_single_value_as_string_with_default(config,LOG_SUBCLASS,LOG_FROM,NULL);
    glyr_opt_from(&q,temp);
    g_free(temp);

#ifdef GLYROS_DEBUG
    g_log(LOG_DOMAIN, G_LOG_LEVEL_DEBUG,"from: %s",q.from);
...
static void pref_construct(GtkWidget * con)
{
    ...
    gtk_container_set_border_width(GTK_CONTAINER(text_box_vbox), 8);
